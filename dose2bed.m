...
SEGM{5,1}='Rectum';
SEGM{5,2}=4;
SEGM{6,1}='Kidney (Left)';
SEGM{6,2}=2;
SEGM{7,1}='Kidney (Right)';
SEGM{7,2}=2;
SEGM{8,1}='SmallBowel';
SEGM{8,2}=4;
SEGM{9,1}='ContralateralBreast';
SEGM{9,2}=3.3;
SEGM{10,1}='Colon';
SEGM{10,2}=4;
SEGM{11,1}='SpinalCanal';
SEGM{11,2}=2;
SEGM{12,1}='SpinalCord';
SEGM{12,2}=2;
SEGM{13,1}='FemoralHead (Right)';
SEGM{13,2}=4;
SEGM{14,1}='Stomach';
SEGM{14,2}=3;
SEGM{15,1}='Lungs';
...
SEGM{18,1}='Alpha6';
SEGM{18,2}=6;
SEGM{19,1}='ThyroidGland';
SEGM{19,2}=3;
SEGM{20,1}='Eye, Left';
SEGM{20,2}=1;
SEGM{21,1}='Eye, Right';
SEGM{21,2}=1;
SEGM{22,1}='ParotidGland (Right)';
SEGM{22,2}=3;
...
DeltaL=round((IPP(3,1)-IPPds(3,1))/Zln);
restoL=(round((IPP(3,1)-IPPds(3,1))/Zln)-((IPP(3,1)-IPPds(3,1))/Zln))*Zln;
...
DeltaLt=round((IPP(1,1)-IPPds(1,1))/Xln);
restoLt=(round((IPP(1,1)-IPPds(1,1))/Xln)-((IPP(1,1)-IPPds(1,1))/Xln))*Xln;
...
DeltaV=round((IPP(2,1)-IPPds(2,1))/Yln);
restoV=(round((IPP(2,1)-IPPds(2,1))/Yln)-((IPP(2,1)-IPPds(2,1))/Yln))*Yln;
